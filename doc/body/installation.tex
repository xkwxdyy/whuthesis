...
\dirtree{%
  .1 /whuthesis.
  .2 logo\DTcomment{存放 logo}.
  .3 whu-logo-colored.png\DTcomment{武汉大学标准校徽\cite{whulogo}}.
  .3 whu-name-black.png\DTcomment{武汉大学黑色校名\cite{whulogo}}.
}
...
% {\color{red}\bfseries 目前 whuthesis 还处于早期的开发状态中，很多功能还并不完善。}

% whuthesis （\WhuLaTeX）宏集意为 a \textcolor{purple}Chinese \textcolor{purple}User
...
% 使用 whuthesis 可以方便地设置标题、目录、页面样式（页面几何元素、页眉页脚等）、图表、背景、水印、
% 边注、脚注、列表、索引、术语表等文档元素，具有强大的可定制性。whuthesis 原生兼容 \pkg{pgf} 和 
% \pkg{tcolorbox}，加载这两个宏包或使用 \whulibrary{pgf} 库可实现更多的功能 \TODO。



% 模块和库均可能加载其它宏包，一般情况下，whuthesis 会自动加载这些模块并处理好它们的依赖和兼容性，
% 当用户需要加载其它宏包时，最好通过 whuthesis 的宏包加载机制来加载它们 \TODO。

% whuthesis 支持 \XeLaTeX、\LuaLaTeX、\upLaTeX、\ApLaTeX（p\LaTeX-ng）等多种编译方式，其中
% \LuaLaTeX、\upLaTeX、\ApLaTeX 还支持竖排 \TODO。

% whuthesis 还很好的支持和适配了通用驱动（generic driver），这是 \LaTeXe 2022-06-01 中的新功能。

% \emph{不}兼容 \pkg{beamer}。

% \chapter{文档接口}

% whuthesis 定义的命令有的用于文档中，有的则是面向开发者，本章描述那些在文档中可能使用到的接口。

% \begin{function}{whuthesis,\WhuLaTeX}
% Logo。输出 whuthesis，\WhuLaTeX。
% \end{function}
...
%     {\includegraphics{whu-aux/tab-defined-papername}}
...
% whuthesis 中排版多栏文字有两种方式，本节描述其中一种，使用 \pkg{multicol} 宏包实现。另一种方式见\cref{sec:lib-box}。
...
% whuthesis 封装了 \pkg{rotating} 宏包，提供了旋转的盒子。
...
% \needspace{3cm}
% \begin{xample}
% \background(lt){\textcolor{red}{\LARGEwhuthesis}}
% \stopxamplecode
% \xampleprint
% 如本页左上角所示。
% \end{xample}
...
% whuthesis 提供了添加多个索引的方法。并且能够自动编译索引文件。
...
% \meta{index keys} 不同于 whuthesis 中其它的键值选项，仅具有类似的接口。
...
% 标准的 {\LaTeX} book 类中，章节标题可分为三种，一种是以 \cs{part} 为代表的，whuthesis 将其命令为 \texttt{page} 类，一种是以 \cs{chapter} 为代表的，whuthesis 将其命名为 \texttt{top} 类，另一种则是以 \cs{section} 为代表，whuthesis 将其命名为 \texttt{normal} 类。\footnote{实际上，这些名称基本沿用了 \pkg{titlesec} 宏包的名称。}
...
% whuthesis 预先定义了一些层级名称，如\cref{tab:level-name} 所示。
% \end{function}
...
% 和 \CTeX 宏集一样，whuthesis 也提供了提供了 \opt{numberformat}，\opt{nameformat}，\opt{titleformat}，\opt{format} 这几个选项用来控制章节标题的格式。它们的作用范围如\cref{fig:heading-format} 所示。
...
% whuthesis 提供了可以控制本章节内的浮动体位置的接口。
...
% whuthesis 重新实现了目录的制作方式，将每个目录项看成是一个个数据，不同于标准的目录制作方式，因此可能存在与其它宏包不兼容的情况。

% 在 whuthesis 中，目录被称为 “combined list”，这是 \ConTeXt 中的称呼。
...
% \begin{function}[module=whu]{\WHULoadLibrary,\WHUIfLibraryAtLeast}
% \begin{syntax}
%   \verb|\WHULoadLibrary|      \marg{库名} \oarg{日期}
%   \verb|\WHUIfLibraryAtLeast| \marg{库名} \marg{日期} \marg{true code} \marg{false code}
% \end{syntax}
% \end{function}
...
% 需要用户自行加载或使用 whuthesis 提供的宏包加载机制来加载。
...
% \nofuncskip 
% \begin{function}[pTF]{\whu_if_preamble:,\whu_if_document:,\whu_if_after_documentclass:}
%   \begin{syntax}
...
% whuthesis 重新实现了标题和目录的命令。其中标题的设置方式是 \CTeX 风格的，
...
% whuthesis 接管各种目录的输出，如标题、图表目录等。如果要输出任何一种目录，
...
% whuthesis 的目录机制适配了许多宏包，诸如 \pkg{algorithm2e}、\pkg{chemmacros}、
...
% \pkg{etoc} 宏包提供了 \tn{etocsetstyle} 命令来设置目录，whuthesis 提供了类似的命令。

% whuthesis 提供了 \csref{tocsetstyle} 和 \csref{specifiedlot} 来分别设置和输出目录，
...
% 下面这个例子展示了本文目录在 whuthesis 目录机制下的输出顺序。
% \begin{xample}
...
% 在 whuthesis 中，如果要输出某类目录，则必须先通过 \csref{addcombinedlisttype} 注册它，
...
% 由于 whuthesis 把所有类型的目录都写入到同一个文件中，因此，\veta{type} 
% 用于唯一区分不同的目录类型。

% 同一个目录中，可以有不同的层级，同一层级也可以有所区分。

% 在 whuthesis 中，用于区分不同层级的
...
% 但在 whuthesis 宏集中，每个 \veta{level name} 只需在其所属的那个目录类型中唯一，
...
% 尽管看起来很复杂，但实际上以上这些工作在绝大多数情况下 whuthesis 都会自动完成它们，
% 无需手动设置它。

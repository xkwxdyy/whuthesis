...
\cs_new_protected:Npn \__whu_bfground_ct:n #1
  {
    \hbox_set:Nn \l__whu_bgfg_box { \hbox_overlap_center:n {#1} }
    \put (\Gm@layouthoffset+.5\Gm@layoutwidth,-\Gm@layoutvoffset)
      { \vbox_to_zero:n { \box_use_drop:N \l__whu_bgfg_box \tex_vss:D } }
  }
\cs_new_protected:Npn \__whu_bfground_rt:n #1
  {
    \hbox_set_to_wd:Nnn \l__whu_bgfg_box { \Gm@layoutwidth } { \tex_hss:D #1 }
    \put (\Gm@layouthoffset,-\Gm@layoutvoffset) 
      { \vbox_to_zero:n { \box_use_drop:N \l__whu_bgfg_box \tex_vss:D } }
  }
...
\cs_new_protected:Npn \__whu_bfground_cb:n #1
  {
    \hbox_set:Nn \l__whu_bgfg_box { \hbox_overlap_center:n {#1} }
...
\cs_new_protected:Npn \__whu_bfground_rm:n #1
  {
    \hbox_set_to_wd:Nnn \l__whu_bgfg_box { \Gm@layoutwidth } { \tex_hss:D #1 }
    \put (\Gm@layouthoffset,
      {-\Gm@layoutvoffset-.5\Gm@layoutheight-.5\box_ht:N \l__whu_bgfg_box+.5\box_dp:N \l__whu_bgfg_box})
...
\cs_new_protected:Npn \__whu_bfground_ot:n 
  {
...
\cs_new_protected:Npn \__whu_bfground_it:n 
  {
    \legacy_if:nTF { @twoside }
      {
        \if_int_odd:w \c@page 
...
\cs_new_protected:Npn \__whu_bfground_im:n 
  {
    \legacy_if:nTF { @twoside }
      {
        \if_int_odd:w \c@page 
          \exp_after:wN \__whu_bfground_lm:n
        \else:
          \exp_after:wN \__whu_bfground_rm:n
        \fi
      }
      { \__whu_bfground_lm:n }
  }
\cs_new_protected:Npn \__whu_bfground_om:n 
  {
    \legacy_if:nTF { @twoside }
      {
        \if_int_odd:w \c@page 
...
\cs_new_protected:Npn \__whu_bfground_ib:n 
  {
    \legacy_if:nTF { @twoside }
      {
        \if_int_odd:w \c@page 
...
\cs_new_protected:Npn \__whu_bfground_ob:n 
  {
    \legacy_if:nTF { @twoside }
      {
        \if_int_odd:w \c@page 

...
\ekeysdeclarecmd \spreadtext { s @O{1} m }
  {
    \tl_set:Nx \__whu_spreadtext_auxii: 
      { 
        \bool_if:nT {#1} { \hss }
        \c_group_end_token 
        \mode_leave_vertical: \box_use_drop:N \l__whu_tmpa_box 
      }
    \tl_set:Nx \l__whu_spreadtext_ratio_tl { \fp_eval:n {#2} }
    \bool_if:nTF {#1}
      { \tex_afterassignment:D \__whu_spreadtext_auxi_hss: }
      { \tex_afterassignment:D \__whu_spreadtext_auxi: }
    \tex_setbox:D \l__whu_tmpa_box 
      \elkernel_hbox:w to \elkernel_dim_expr:w #3 \scan_stop:
  }
\ekeysdeclarecmd \useinfiniteskip { m } { \__whu_spreadtext_set_skip:n {#1} }
\tl_new:N \l__whu_spreadtext_ratio_tl
\cs_new:Npn \__whu_spreadtext_auxi:
  {
    \__whu_spreadtext_set_skip:n { \l__whu_spreadtext_ratio_tl fil }
    \c_group_begin_token
    \group_insert_after:N \__whu_spreadtext_auxii:
  }
\cs_new:Npn \__whu_spreadtext_auxi_hss:
  {
    \__whu_spreadtext_set_skip:n { \l__whu_spreadtext_ratio_tl fil }
    \hss
    \c_group_begin_token
...
\bool_case:nF 
  {
    \sys_if_engine_uptex_p:
      {
        \cs_new_protected:Npn \__whu_spreadtext_set_skip:n #1
          {
...
\cs_new:Npn \__whu_spreadtext_leaders:nn #1 { \tex_xleaders:D #1 }

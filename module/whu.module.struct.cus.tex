...
\socket_new_plug:nnn { templatecbl } { by~name }
  {
    ...
    \group_begin:
      ...
      \tmcbl@pushdef \tmcblthetype
        {
          \bgroup \__whu_tmcbl_info:w ##3, \s__whu_mark
          \use:e { \UseInstance { templatecbl } { \tmcbltheclass } } 
            {##1}{##2}{##3}{##4}{##5}{##6}{##7}
        }
        { \egroup }
      \whu@tmcbl@getrange #5,,\@nil {#1}
...
\cs_new:Npn \whu@tmcbl@getrange #1, #2, #3 \@nil #4
  {
...
\cs_new_protected:Npn \__whu_tmcbl_fix_function:
  {
    ...
    \clist_map_function:NN \tmcbl@hyperrangelist \__whu_tmcbl_hyper_range:n

...
\WHUDependency{ disable={titlesec,titletoc,tocloft,etoc,placeins,multitoc,minitoc}, 
  module={util.cus} }
\WHUProvideExplModule{struct.cus}{\whu@d@te}{\whu@versi@n}{structures of document}
...
\tl_new:N \g__whu_contents_patch_tl
\tl_gput_right:Nn \g__whu_contents_patch_tl 
  {
...
\whu_pkg_code:nnn { biblatex } { }
  { \tl_gput_right:Nn \g__whu_contents_patch_tl { \blx@contentssafe@citecommands } }
...
\cs_new_protected:Npn \whu_contents_line:nnnnn #1#2#3#4#5 
  {
    ...
    \__whu_cbl_parse_title:n {#3} % save to \l__whu_tmpc_tl
    \tl_gset:cx 
      { whu/cbl~/cbl~ /entry [ \int_use:N \g__whu_cbl_count_int ] _tl }
      {
        \exp_not:N \whu@cbl@contentsline {#1} 
          ...
          { \exp_not:o \l__whu_tmpc_tl } {#4} {#5} \exp_not:N \whu@cbl@contentsline@
      }
    \tl_gset:cx 
      { whu/cbl~/type~#1 /entry [ \int_use:c { g__whu_cbl_type~#1_count_int } ] _tl }
      {
        \exp_not:N \whu@type@contentsline {#1}
          ...
          {#2} { \l__whu_cbl_heading_level_tl }
          { \exp_not:o \l__whu_tmpc_tl } {#4} {#5} \exp_not:N \whu@type@contentsline@
      }
...
\cs_new_protected:Npn \__whu_cbl_parse_title:n #1
  {
    \tl_if_head_eq_meaning:nNTF {#1} \numberline
      { \tl_set:No \l__whu_tmpc_tl { \__whu_cbl_parse_title:nnw #1 \s__whu_stop } }
      {
        \tl_if_in:nnTF {#1} { \hspace }
          { \tl_set:Nx \l__whu_tmpc_tl { \__whu_cbl_parse_title:ww #1 \s__whu_stop } }
          { \tl_set:Nn \l__whu_tmpc_tl { \nonumberline{}{#1} } }
      }
  }
\cs_new_protected:Npn \nonumberline #1#2 {#1#2}
\cs_new:Npn \__whu_cbl_parse_title:nnw #1#2#3 \s__whu_stop { #1{#2}{#3} }
\cs_new:Npn \__whu_cbl_parse_title:ww #1 \hspace #2#3 \s__whu_stop 
  { 
    \nonumberline 
      { \exp_not:n {#1} 
        \str_if_eq:nnTF {#2} { * } 
...
\cs_new_protected:Npn \__whu_contentsline_push_def:nnn #1#2#3
  {
    ...
    \tl_if_empty:eTF {#1}
      {
        ...
        \cs_gset_eq:cN 
          ...
          \whu@cbl@contentsline@
        \cs_set:Npn \whu@cbl@contentsline ##1##2##3##4##5##6##7 {#2}
        \cs_set:Npn \whu@cbl@contentsline@ {#3}
      }
      {
        \cs_gset_eq:cN 
          { __whu_iterate_contents_ \int_use:N \g__whu_map_int :w }
          \whu@type@contentsline
        \cs_gset_eq:cN 
          { __whu_iterate_contents@_ \int_use:N \g__whu_map_int :w }
          \whu@type@contentsline@
        \cs_set:Npn \whu@type@contentsline ##1##2##3##4##5##6##7 {#2}
        \cs_set:Npn \whu@type@contentsline@ {#3}
      }
  }
\cs_new_protected:Npn \__whu_contentsline_pop_def:n #1
  {
    \tl_if_empty:eTF {#1}
      {
      ...
      {
        \cs_set_eq:Nc \whu@type@contentsline
          { __whu_iterate_contents_ \int_use:N \g__whu_map_int :w }
        \cs_set_eq:Nc \whu@type@contentsline@
          { __whu_iterate_contents@_ \int_use:N \g__whu_map_int :w }
      }
...
\cs_new_protected:Npn \whu_map_heading_level:nn #1#2
  {
    \int_gincr:N \g__whu_map_int
    \cs_set_protected:cpn { __whu_map_tmp_ \int_use:N \g__whu_map_int :w } ##1##2 {#2}
    \tl_if_empty:eTF {#1}
      {
        \exp_args:NNe \prop_map_tokens:Nn \g__whu_heading_level_prop
          {
            \__whu_map_heading_level_aux:Nnn
            \cs:w __whu_map_tmp_ \int_use:N \g__whu_map_int :w \cs_end:
          }
      }
      {
        \exp_args:NNe \prop_map_tokens:Nn \g__whu_heading_level_prop
          {
            \__whu_map_heading_level_aux:Nnnn 
            \cs:w __whu_map_tmp_ \int_use:N \g__whu_map_int :w \cs_end: {#1}
          }
      }
    \int_gdecr:N \g__whu_map_int
  }
\cs_new_protected:Npn \__whu_map_heading_level_aux:Nnn #1#2#3
  { \exp_after:wN #1 \__whu_map_heading_level_aux:w #2 \s__whu_stop }
\cs_new_protected:Npn \__whu_map_heading_level_aux:Nnnn #1#2#3#4
  {
    \str_if_eq:eeT {#2}
      { \exp_after:wN \use_i:nn \__whu_map_heading_level_aux:w #3 \s__whu_stop }
      { \exp_after:wN #1 \__whu_map_heading_level_aux:w #3 \s__whu_stop }
  }
\cs_new:Npn \__whu_map_heading_level_aux:w #1 - #2 \s__whu_stop {{#1}{#2}}
...
\prop_new_linked:N \g__whu_heading_level_prop
...
\cs_new_protected:Npn \tmcbl@pushdef { \__whu_contentsline_push_def:nnn }
\cs_new_protected:Npn \tmcbl@popdef { \__whu_contentsline_pop_def:n }
\keys_define:nn { whu/templatecbl }
  {
    no-parent .code:n = \__whu_tmcbl_remove_first_entry: ,
    no-parent .value_forbidden:n = true ,
    hide .code:n = \__whu_tmcbl_set_instances_keys:nn {#1} { ignore=true } ,
    show .code:n = \__whu_tmcbl_set_instances_keys:nn {#1} { ignore=false } ,

    * .code = \__whu_tmcbl_edit_instance:n {#1} ,
    unknown .code:n = 
      \tl_if_head_eq_charcode:oNTF \l_keys_key_str *
        { 
          \exp_last_two_unbraced:Noo \__whu_tmcbl_edit_instance:wn
        ...
        }
        {
          \exp_args:Nno \IfInstanceExistTF { templatecbl } \l_keys_key_str
            ...
            { \msg_error:nnx { whu } { unknown-templatecbl-key } \l_keys_key_str }
        } ,
  }
\cs_new_protected:Npn \tmcbl@getinfo #1 {  \__whu_tmcbl_info:w #1, \s__whu_mark }
\cs_new:Npn \__whu_tmcbl_info:w #1, #2 \s__whu_mark
  {
...
\cs_new_protected:Npn \tmcbl@getrange #1#2
  { \__whu_tmcbl_get_range:wwn #2,,\s__whu_stop {#1} }
\cs_new:Npn \__whu_tmcbl_get_range:wwn #1, #2, #3 \s__whu_stop #4
  {
...
\cs_new_protected:Npn \__whu_tmcbl_remove_first_entry:
  {
...
\cs_new_protected:Npn \__whu_tmcbl_edit_instance:n #1
  {
    \whu_map_heading_level:nn \tmcblthetype
      {
...
\use:e 
  {
    \cs_new_protected:Npn \exp_not:N \__whu_tmcbl_edit_instance:wn 
      * #1 \c_left_brace_str #2 \c_right_brace_str
  }
  { \__whu_tmcbl_set_instances_keys:nn {#2} }
\cs_new_protected:Npn \__whu_tmcbl_set_instances_keys:nn #1#2
  { \clist_map_inline:nn {#1} { \EditInstance { templatecbl } {##1} {#2} } }
\msg_new:nnn { whu } { unknown-templatecbl-key }
...
\cs_new_protected:Npn \tmcbl@hyperlink { \whu_ref_target:nn {\tmcbltheanchor} }
\cs_new_protected:Npn \tmcbl@hyperlinkbox { \whu_ref_target_box:nn {\tmcbltheanchor} }
...
\DeclareTemplateCode { templatecbl } { by~name } { 7 }
  ...
  }
  {
    \__whu_tmcbl_keys:
    \AssignTemplateKeys
    \__whu_tmcbl_fix_function:
    % type, . count, info, level, list entry, thepage, anchor
    \tmcbl@code {#1}{#2}{#3}{#4}{#5}{#6}{#7}
  }
\cs_new_protected:Npn \__whu_tmcbl_keys: { }
\cs_new:Npn \__whu_tmcbl_hyper_range:n #1
  { \cs_if_exist_use:c { __whu_tmcbl_hyper_range_#1: } }
\cs_new:Npn \__whu_tmcbl_hyper_range_name:  { \int_add:Nn \tmcbl@hyperrange { 1 } }
\cs_new:Npn \__whu_tmcbl_hyper_range_title: { \int_add:Nn \tmcbl@hyperrange { 2 } }
\cs_new:Npn \__whu_tmcbl_hyper_range_page:  { \int_add:Nn \tmcbl@hyperrange { 4 } }
\cs_new:Npn \__whu_tmcbl_hyper_range_none:  { \int_set:Nn \tmcbl@hyperrange { 0 } }
\cs_new:Npn \__whu_tmcbl_hyper_range_all:   { \int_set:Nn \tmcbl@hyperrange { 7 } }
\cs_new_protected:Npn \tmcbl@parsehyperrange #1
  {
    \clist_remove_duplicates:N #1
    \int_zero:N \tmcbl@hyperrange
    \clist_map_function:NN #1 \__whu_tmcbl_hyper_range:n
  }
\cs_new_protected:Npn \__whu_tmcbl_fix_function:
  {
        ...
        \tl_if_blank:nTF {##4}
          {
            \whu_if_keyval_variable:nNnn {##3}
              ...
              {
                \__whu_title:onnnnn \l__whu_title_curr_keys_tl
                  {#2} {#3} {##1} {##5} {##5}
              }
              {
                \tl_if_empty:NTF \l__whu_title_curr_keys_tl
                  { \__whu_title:nnnnnn { } {#2} {#3} {##1} {##5} {##5} }
                  { \__whu_title:nnnnnn { } {#2} {#3} {##1} {##3} {##5} }
              }
          }
          { \__whu_title:nnnnnn {##3} {#2} {#3} {##1} {##4} {##5} }
        \__whu_restore_title_setting:n {#2}
        \__whu_title_ending:
      }
  }
\tl_new:N \g__whu_title_next_keys_tl
\bool_new:N \g__whu_title_next_known_bool
% key, title str, class, star or else, entry, title 
\cs_new_protected:Npn \__whu_title:nnnnnn #1#2 
  {
    \exp_args:Nno \__whu_store_title_setting:nn 
      {#2} { \g__whu_title_next_keys_tl , #1 }
    \tl_if_blank:oTF \g__whu_title_next_keys_tl
      { \whu_setup_title:nn {#2} {#1} }
      {
        \bool_if:NTF \g__whu_title_next_known_bool
          {
            \exp_args:Nno \whu_setup_known_title:nn 
              {#2} { \g__whu_title_next_keys_tl , #1 }
          }
          {
            \exp_args:Nno \whu_setup_title:nn 
              {#2} { \g__whu_title_next_keys_tl , #1 }
          }
      }
    \tl_gclear:N \g__whu_title_next_keys_tl
    \bool_gset_false:N \g__whu_title_next_known_bool
    \__whu_title:nnnnn {#2}
  }
\cs_new_protected:Npn \__whu_title:onnnnn { \exp_args:No \__whu_title:nnnnnn }
% title str, class, star or else, entry, title 
\cs_new_protected:Npn \__whu_title:nnnnn #1#2#3#4#5
  {
    \use:e 
      { 
        \exp_not:n {#2} 
...
\ekeysdeclarecmd \setupnexttitle { t? t+ m }
  {
    \bool_gset:Nn \g__whu_title_next_known_bool {#1}
    \bool_if:nTF {#2} \tl_gput_right:Nn \tl_gset:Nn \g__whu_title_next_keys_tl {,#3}
  }
...
\clist_new:N \l__whu_title_unknown_keys_clist
\cs_new_protected:Npn \whu_setup_known_title:nn #1 
  { \keys_set_known:nnN { whu/title/#1 } \l__whu_title_unknown_keys_clist }
    ...
    \keys_define:nn { whu/title/ #1 }
      {
        ...
        numbering / on  .code:n = \__whu_title_set_mode:nn {#1} { normal } ,
...
% 这通过 \__whu_store_title_setting:nn 和 \__whu_restore_title_setting:n 完成
% 如果需要特殊的处理，可以使用
% \whu_set_title_key_store_code:nn 和 \whu_set_title_key_restore_code:nn 
...
\cs_new_protected:Npn \whu_setup_title_store_restore:nn #1
  { 
    \keyval_parse:nnn 
      { \__whu_setup_title_store_restore:nn {#1} } % 如果没有值，则表示忽略该键
...
\cs_new_protected:Npn \whu_setup_title_store_restore:nnn #1#2#3
  {
    \clist_map_inline:nn {#2}
      { \__whu_setup_title_store_restore_by:nnn {#1} {##1} {#3} }
  }
% 设置标题通用的恢复方法
\cs_new:Npn \whu_setup_title_store_restore:n { \whu_setup_title_store_restore:nn { } }
\cs_new:Npn \__whu_setup_title_store_restore:nn #1#2 % title,key
  { \__whu_setup_title_store_restore:nnnn {#1} {#2} { } { } }
\cs_new:Npn \__whu_setup_title_store_restore:nnn #1#2#3 % title,key,store restore code
  {
    % \bool_lazy_and:nnTF 
    %   { \tl_if_head_is_group_p:n {#3} }
    %   { \int_compare_p:nNn { 2 } = { \tl_count:n {#3} } }
    \tl_if_head_is_group:nTF {#3}
      { \__whu_setup_title_store_restore:nnnn {#1} {#2} #3 } % 直接设置为它们
      { \__whu_setup_title_store_restore_by:nnn {#1} {#2} {#3} }
  }
...
\cs_new_protected:Npn \__whu_setup_title_store_restore_by:nnn #1#2#3
  { \use:c { __whu_setup_title_store_restore_#3:nn } {#1} {#2} }
\cs_new_protected:Npn \__whu_setup_title_store_restore_:nn #1#2 % none
  { \__whu_setup_title_store_restore:nnnn {#1} {#2} { } { } }
\cs_new_eq:NN 
  \__whu_setup_title_store_restore_none:nn
  \__whu_setup_title_store_restore_:nn
\cs_new_protected:Npn \__whu_setup_title_store_restore_general:nn #1#2
  {
    \cs_set_eq:cc { title~store@#1@#2@command } { title~store@@#2@command }
    \cs_set_eq:cc { title~restore@#1@#2@command } { title~restore@@#2@command }
  }
\cs_new_protected:Npn \__whu_setup_title_store_restore_default:nn #1#2
  {
...
\whu_setup_title_store_restore:n 
  {
    ...
    , beforerecord<=
      {
        \seq_put_right:Nn \l__whu_title_restore_key_seq { beforerecord }
    ...
    , properties+=
    {
      \seq_put_right:Nn \l__whu_title_restore_key_seq { properties }
    ...
    , nameformat+=
      {
        \seq_put_right:Nn \l__whu_title_restore_key_seq { nameformat }
    ...
    , numberformat+=
      {
        \seq_put_right:Nn \l__whu_title_restore_key_seq { numberformat }
    ...
    , titleformat+=
      {
        \seq_put_right:Nn \l__whu_title_restore_key_seq { titleformat }
    ...
    , aftername+=
      {
        \seq_put_right:Nn \l__whu_title_restore_key_seq { aftername }
    ...
    , aftertitle+=
      {
        \seq_put_right:Nn \l__whu_title_restore_key_seq { aftertitle }
...
\NewDocumentCommand \settocdepth { >{\TrimSpaces} m }
  { 
    \whu_if_head_int:nTF {#1} { \setcounter { tocdepth } { \int_eval:n {#1} } }
      { \exp_args:Nnf \setcounter { tocdepth } { \whu_heading_level:n { toc-#1 } } }
  }
...
\cs_new:Npn \title@bookmarkextra
  { 
    \tl_if_empty:cF { title~parameter@ \CurrentTitleName @bookmark-extra }
      {
        \exp_last_unbraced:Ne \whu_bookmark:nn 
          { 
            \exp_args:Nv \__whu_title_bookmark:n 
              { title~parameter@ \CurrentTitleName @bookmark-extra } 
          }
        \tl_clear:c { title~parameter@ \CurrentTitleName @bookmark-extra }
      }
  }
\cs_new:Npn \__whu_title_bookmark:n #1
  { \exp_last_unbraced:Ne \__whu_title_bookmark:wn { \whu_split_bracket_head:n {#1} } }
\cs_new:Npn \__whu_title_bookmark:wn [#1] #2 { {#1} {#2} }

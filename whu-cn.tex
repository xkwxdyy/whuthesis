...
\whusetstyle*[frame]{frame color}{\colorlet{frame color}{#1}}
\whusetstyle*[frame]{fill color}{\colorlet{fill color}{#1}}
\whusetstyle[frame]{color frame}{%
  frame={\setlength{\fboxsep}{#1\whuframesep}%
    \setlength{\fboxrule}{#1\whuframerule}%
...
\begin{xample}
\ttfamily \csstring\WhuTeX \quad \csstring\|
\stopxamplecode
\xampleprint
\end{xample}
...
\section{盒子和填充，\whumodule{box}模块}

\whumodule{box} 用于提供盒子构造、内容填充等内容。
...
\whumodule{box} 模块定义了一个简易的可跨页的盒子环境 \env{Framed}，相较于 
...
\begin{keyval}[path=frame]{sep}
  ...
  \end{syntax}
设置变量 \tn{whuframesep}，即盒子外框与内容的间距。
\end{keyval}
...
\whumodule{box} 提供了几个创建 filler 的命令。
...
\begin{keyval}[path=filler]{color}
  \begin{syntax}
    color = <{color expr}>
  \end{syntax}
设置颜色 \key[module=color name]{whufiller}，即填充的颜色。
\end{keyval}
...
\WhuTeX 中排版多栏文字有两种方式，本节描述其中一种，使用 \pkg{multicol} 宏包实现。另一种方式见\cref{sec:lib-box}。
...
\whumodule{tcb} 库提供 \opt{tcbox} 值（\cref{sec:multicolumns/framed=tcbox}），
\whumodule{box} 库提供 \opt{lfbox} 值（\cref{sec:multicolumns/framed=lfbox}）。
...
\WhuTeX 封装了 \pkg{rotating} 宏包，提供了旋转的盒子。
...
\section{背景，\whumodule{bgfg}模块}

\whumodule{bgfg} 是对 \texttt{shipout} 钩子的简单封装。
...
\meta{index keys} 不同于 \WhuTeX 中其它的键值选项，仅具有类似的接口。
...
\begin{xample}
\newindextype[
  filename=\jobname.dowhuage.idx, 
  output=\jobname.dowhuage.ind,
...
\section{文档结构，\whumodule{struct}模块}

\whumodule{struct} 模块提供了创建目录和章节标题的方法。参考了 \pkg{titlesec}，
...
标准的 {\LaTeX} book 类中，章节标题可分为三种，一种是以 \cs{part} 为代表的，\WhuTeX 将其命令为 \texttt{page} 类，一种是以 \cs{chapter} 为代表的，\WhuTeX 将其命名为 \texttt{top} 类，另一种则是以 \cs{section} 为代表，\WhuTeX 将其命名为 \texttt{normal} 类。\footnote{实际上，这些名称基本沿用了 \pkg{titlesec} 宏包的名称。}
...
\WhuTeX 预先定义了一些层级名称，如\cref{tab:level-name} 所示。
\end{function}
...
和 \CTeX 宏集一样，\WhuTeX 也提供了 \opt{numberformat}，\opt{nameformat}，\opt{titleformat}，\opt{format} 这几个选项用来控制章节标题的格式。它们的作用范围如\cref{fig:heading-format} 所示。
...
写入的 \veta{label} 名为 \texttt{whu.title.}\cs{thetitlecount}。

可以使用 \verb|\RefProperty{whu.title.|\cs{thetitlecount}\verb|}|\marg{property} 获得
...
\WhuTeX 重新实现了目录的制作方式，将每个目录项看成是一个个数据，不同于标准的目录制作方式，因此可能存在与其它宏包不兼容的情况。

在 \WhuTeX 中，目录被称为 “combined list”，这是 \ConTeXt 中的称呼。

\WhuTeX 的 \whumodule{struct} 一共定义了三种格式的目录：plain、template、specified。
...
\pkg{whu} 提供了与 \cls{book} 类相似的目录格式，这种目录称为 ``plain'' 格式的目录。
...
\pkg{whu} 提供了设置用键值对目录进行简易调整的目录格式，称为 ``template'' 格式的目录。
...
\cs{templatetoc} 等命令内部使用，不能在外部的 \cs{whusetup} 中设置。
...
\texttt{by name} 模板可用的键如下（注意它们不能用 \cs{whusetup} 设置，且每个键都必须给出
...
%region buffer, TODO
\section{\whumodule{buffer}模块}
未完成。\TODO 

% \whumodule{buffer} 用于提供缓存机制，将内容保存起来，以便后续使用，例如 verbatim。
...
\begin{syntax}
  \verb|\WHUProvideLibrary|     \marg{库名} \oarg{描述}
  \verb|\WHUProvideExplLibrary| \marg{库名} \marg{日期} \marg{版本} \marg{描述}
\end{syntax}
提供库文件。库文件名必须是：“\opt{whu.library.\meta{库名}.tex}”。
\end{function}

\begin{function}[module=whu]{\WHULibraryDelayedUntil}
  \begin{syntax}
    \V\WHULibraryDelayedUntil   \marg{package}
...
\begin{syntax}
  \verb|\WHUDependency|   \marg{key-val}
  \verb|\WHUDependency| * \marg{key-val}
\end{syntax}
处理库依赖。
...
\begin{syntax}
  ...
  disable = \marg{clist}
\end{syntax}
\cs{WHUDependency} 可用的键值选项。
\end{keyval}

\begin{texnote}
只能以如下顺序执行上述三个命令：\cs{WHULibraryDelayedUntil}、\linebreak
\cs{WHUDependency}、\cs{WHUProvideExplLibrary}（或 \cs{WHUProvideLibrary}）。
在每个库文件中，它们最多只能使用一次，且必须用在库文件开头。
若要在库文件中间使用它们，则应该将之后的内容拆分至新的库文件中。

一个库文件可能会被使用多次，但在 \cs{WHUProvideExplLibrary}（或 \cs{WHUProvideLibrary}） 
之后的内容只会执行一次。
\end{texnote}

\begin{function}[module=whu]{\WHULoadLibrary,\WHUPassOptionsToLibrary}
\begin{syntax}
  \V\WHULoadLibrary \oarg{选项} \marg{库名} \oarg{日期}
  \V\WHUPassOptionsToLibrary \marg{选项} \marg{库名}
\end{syntax}
尽量不要使用 \cs{WHULoadLibrary}，它可能不会正确处理库依赖。
\end{function}
...
\nofuncskip
\begin{function}[EXP]{\whu@filename}
  \begin{syntax}
    \V*|\whu@filename| \marg{filename}
  \end{syntax}
...
\begin{function}{\whu@getgraphicsname}
  \begin{syntax}
    \V*|\whu@getgraphicsname| \meta{cmd} \marg{filename}
  \end{syntax}
...
\begin{function}[noTF]{\whu_get_graphics_full_name:nN,}
  \begin{syntax}
    \V*|\whu_get_graphics_full_name:nN|   \marg{filename} \meta{tl}
    \V*|\whu_get_graphics_full_name:nNTF| \marg{filename} \meta{tl} \marg{true code} \marg{false code}
  \end{syntax}
...
\section{\whumodule{util}模块}\label{sec:module-util}
...
需要用户自行加载或使用 \WhuTeX 提供的宏包加载机制来加载。

\begin{function}[EXP]{\labelinfo,\whu_label_info:nn}
  \begin{syntax}
...
\begin{function}{\whu_newlabel_now:nnnnnn,\whu_newlabel_now:xxxxxx,
  ...
  \begin{syntax}
    \V*|\whu_new_label_now:nnnnnn| \marg{label} \marg{ref data} \marg{thepage} 
...
\begin{function}{\whu_make_target:n,\whu_make_unique_target:n}
  \begin{syntax}
    \V*|\whu_make_target:n| \marg{target}
    \V*|\whu_make_unique_target:n| \marg{target}
  \end{syntax}
\cs{whu_make_target:n} 以 \meta{target} 为名，创建一个锚点。\meta{target} 必须唯一。锚点位置自动升高 \tn{normalbaselineskip}。

\cs{whu_make_unique_target:n} 创建一个锚点，其名以 \meta{target} 为前缀，由一个共享的计数器保证这个锚点名唯一，每调用一次，这个计数器都会自增。
...
\begin{function}{\g_whu_anchor_tl,\whu_gset_next_anchor_name:n,\whu_gset_next_anchor_raise:n}
  \begin{syntax}
    \V*|\whu_gset_next_anchor_name:n|  \marg{name}
    \V*|\whu_gset_next_anchor_raise:n| \marg{dim}
  \end{syntax}
\cs{g_whu_anchor_tl} 保存了最近一个锚点的名称，它是只读的。相当于 \tn{@cur\-rentHref}。

\cs{whu_gset_next_anchor_name:n} 设置下一个锚点的名称。\meta{name} 被立刻展开。

\cs{whu_gset_next_anchor_raise:n} 设置下一个锚点要升高的高度。\meta{dim} 立即被计算。
...
\begin{function}{\whu_ref_label:nn,\whu_ref_target:nn,\whu_ref_label_box:nn,\whu_ref_target_box:nn}
  \begin{syntax}
    \V*|\whu_ref_label:nn|  \marg{label} \marg{text}
    \V*|\whu_ref_target:nn| \marg{target} \marg{text}
    \V*|\whu_ref_label_box:nn|  \marg{label} <material>
    \V*|\whu_ref_target_box:nn| \marg{target} <material>
  \end{syntax}
...
另见 \cs{whu_ref_label_shbox:nn}、\cs{whu_ref_target_shbox:nn}。
...
\begin{xample}
\ExplSyntaxOn
\whu_ref_label_box:nn { sec:module-util } \bgroup\verb|本节开始|\egroup 或
\whu_ref_label:nn { sec:module-util } { 链接到本节开始，但是是很长很长很长很长很长很长的可以断行的文字。 }
\ExplSyntaxOff
\stopxamplecode
\xampleprint 
\end{xample}

\begin{function}{\whu@colorfile,\whu@colorlink,\whu@colorcite,
  \whu@colorurl,\whu@colorrun,\whu@colormenu}
把文字的颜色改为对应的颜色。

注意：使用 \verb|\hypersetup{linkcolor=red}| 等修改的颜色仅在 PDF 阅读器中显示，在打印时不会显示，而使用上述 6 个命令以及 \tn{color}、\cs{color_select:n} 等修改的颜色会在打印时显示。
\end{function}

\begin{function}{\whu_bookmark:nn,\whu_gset_next_bookmark_text:n}
  \begin{syntax}
    \V*|\whu_bookmark:nn| \marg{options} \marg{bookmark}
    \V*|\whu_gset_next_bookmark_text:n| \marg{bookmark}
  \end{syntax}
设置书签。或设置下一个书签的文字。

\cs{whu_bookmark:nn} 是对 \tn{bookmark} 的封装。加载了 \pkg{bookmark} 宏包才有效。
\end{function}

\begin{function}[pTF]{\whu_if_pdfstring:}
  \begin{syntax}
    \V*|\whu_if_pdfstring:TF| \marg{true} \marg{false}
  \end{syntax}
判断是否在书签等 PDF 字段内。
\end{function}

\begin{function}{\whu_hyper_anchor:n,\whu_hyper_anchor_start:n,\whu_hyper_anchor_stop:,
  \whu_hyper_link:nnn,\whu_hyper_link_start:nn,\whu_hyper_link_stop:,
  \whu_hyper_link_file:nnn,\whu_hyper_link_url:nn,
  \whu_hyper_link_launch:nnn,\whu_hyper_link_name:nn}
  \begin{syntax}
    \V*|\whu_hyper_anchor:n|       \marg{destination name}
    \V*|\whu_hyper_anchor_start:n| \marg{destination name} \meta{content}
    ~~~~\V*|\whu_hyper_anchor_stop:|
    \V*|\whu_hyper_link:nnn|      \marg{context} \marg{destination name} \marg{link text}
    \V*|\whu_hyper_link_start:nn| \marg{context} \marg{destination name} \meta{content}
    ~~~~\V*|\whu_hyper_link_stop:|
    \V*|\whu_hyper_link_file:nnn| \marg{link text} \marg{filename} \marg{destname}
    \V*|\whu_hyper_link_url:nn|   \marg{link text} \marg{url}
    \V*|\whu_hyper_link_launch:nnn| \marg{filename} \marg{link text} \marg{Parameters}
    \V*|\whu_hyper_link_name:nn|    \marg{action} \marg{link text}
  \end{syntax}
...
\begin{function}{\whu_gset_next_page_label:n,
  \whu_gset_page_label:n,\whu_gset_page_label_code:n,\whu_reset_page_label_code:}
  \begin{syntax}
    \V*|\whu_gset_next_page_label:n| \marg{page label}
    \V*|\whu_gset_page_label:n|      \marg{page label}
    \V*|\whu_gset_page_label_code:n| \marg{code}
    \V*|\whu_reset_page_label_code:|
  \end{syntax}
这些命令用于设置在阅读器中显示的页码。\meta{page label} 一般包含 \tn{thepage} 或
\verb|\arabic{page}| 等内容。\cs{whu_gset_next_page_label:n} 相当于 \pkg{hyperref}
...
% 相当于下面这段代码
% \whu_gset_page_label_code:n 
%   {
%     \tl_set:Nn #1 { SP.\thepage }
%     \whu_reset_page_label_code: 
%   }
\ExplSyntaxOff
\stopxamplecode
...
\begin{function}{\whu_peek_verbatim:nw}
  \begin{syntax}
    \V*|\whu_peek_verbatim:nw| \marg{tokens_1} \marg{balanced tokens}
    \V*|\whu_peek_verbatim:nw| \marg{tokens_1} \meta{token} \meta{tokens} \meta{token}
  \end{syntax}
...
\begin{function}[pTF]{\whu_if_head_int:n}
  \begin{syntax}
    \verb|\whu_if_head_int:nTF| \marg{tl} \marg{true code} \marg{false code}
  \end{syntax}
测试 \meta{tl} 是否以数字起始。
\end{function}
...
\begin{xample}
\ExplSyntaxOn
  1.  \whu_if_head_int:nTF { 2022  } { T } { F }
\ 2.  \whu_if_head_int:nTF { +2022 } { T } { F } % 正数
\ 3.  \whu_if_head_int:nTF { -2022 } { T } { F } % 负数
\ 4.  \whu_if_head_int:nTF { '3746 } { T } { F } % 8进制数
\ 5.  \whu_if_head_int:nTF { "7E6  } { T } { F } % 16进制数
\ 6.  \whu_if_head_int:nTF { Notnu } { T } { F }
\ 7.  \whu_if_head_int:nTF { \par  } { T } { F }
\ 8.  \whu_if_head_int:nTF { \c_true_bool } { T } { F } % \char
\ 9.  \whu_if_head_int:nTF { \c_one_int   } { T } { F } % int (count)
\ 10. \whu_if_head_int:nTF { \tracingmacros } { T } { F }
\ 11. \whu_if_head_int:nTF { \the\tracingmacros } { T } { F } % 展开为整数
\ 12. \whu_if_head_int:nTF { \baselineskip } { T } { F }
\ 13. \whu_if_head_int:nTF { \number\baselineskip } { T } { F } % 展开为整数
\ExplSyntaxOff
\stopxamplecode
\xampleprint
\end{xample}

\begin{function}{\whu_arg_to_keyval_apply:nnN,\whu_arg_to_keyval_apply:nnn}
  \begin{syntax}
    \V*|\whu_arg_to_keyval_apply:nnN| \marg{key name} \marg{arg} \meta{function}
    \V*|\whu_arg_to_keyval_apply:nnn| \marg{key name} \marg{arg} \marg{tokens}
  \end{syntax}
判断 \meta{arg} 是否为键值对；如果不是则把 \meta{key name} 作为键名，\meta{arg} 作为值，
形成一个键值对。再把这键值放在 \meta{function}（或 \meta{tokens}） 的后面。
\end{function}

\begin{function}{\whu_if_keyval_apply:nNN,\whu_if_keyval_apply:nnn}
  \begin{syntax}
    \V*|\whu_if_keyval_apply:nNN| \marg{arg} \meta{true function} \meta{false function}
    \V*|\whu_if_keyval_apply:nnn| \marg{arg} \marg{true tokens} \marg{false tokens}
  \end{syntax}
...
\begin{function}{\whu_if_keyval_variable:nNnn}
  \begin{syntax}
    \V*|\whu_if_keyval_variable:nNnn| \marg{arg} \meta{variable} \marg{true code} \marg{false code}
  \end{syntax}
...
\cs_new_protected:Npn \fiicmd #1
  {
    \whu_if_keyval_variable:nNnn {#1} \l_tmpa_tl
...
\begin{syntax}
  \verb|\whu_split_bracket_head_default:nn| \marg{default} \marg{tl}
  \verb|\whu_split_bracket_head:n|                   \;\marg{tl}
\end{syntax}
...
\exp_args:Ne \tl_to_str:n
  {
    ...
    \space 2. \whu_split_bracket_head:n { tail }
    \space 3. \whu_split_bracket_head:n { [bracket]tail }
    \space 4. \whu_split_bracket_head:n { [bracket] }
  }
\ExplSyntaxOff
\stopxamplecode
\xampleprint
...
\begin{syntax}
  \verb|\whu_split_bracket_tail_default:nn| \marg{tl} \marg{default}
  \verb|\whu_split_bracket_tail:n|          \marg{tl}
\end{syntax}
...
\exp_args:Ne \tl_to_str:n
  {
    ...
    \space 2. \whu_split_bracket_tail:n { head }
    \space 3. \whu_split_bracket_tail:n { head[bracket] }
    \space 4. \whu_split_bracket_tail:n { [bracket] }
  }
\ExplSyntaxOff
\stopxamplecode
\xampleprint
...
\begin{function}[EXP]{\whu_tl_split_braced:nn}
  \begin{syntax}
    \V*|\whu_tl_split_braced:nn|   \marg{tl_1} \marg{tl_2}
  \end{syntax}
\cs{whu_tl_split_braced:nn} 提取 \meta{tl_1} 和 \meta{tl_2} 的值，将它们分别拆成两个部分，
...
\begin{function}{\whu_tl_split_braced:NNNN}
  \begin{syntax}
    \V*|\whu_tl_split_braced:NNNN| <tl var_1> <tl var_2> <tl var_3> <tl var_3>
  \end{syntax}
\cs{whu_tl_split_braced:NNNN} 提取 \meta{tl var_1} 和 \meta{tl var_2} 的值，
...
\begin{xample}
\ttfamily \ExplSyntaxOn
\tl_to_str:e { \whu_tl_split_braced:nn {12345} {ABCDEFG} } \par 
\tl_set:Nn \l_tmpa_tl { 12345   }
\tl_set:Nn \l_tmpb_tl { ABCDEFG }
\whu_tl_split_braced:NNNN \l_tmpa_tl \l_tmpb_tl \l_tmpc_tl \l_tmpd_tl
...
\nofuncskip 
\begin{function}[pTF]{\whu_if_preamble:,\whu_if_document:,
  \whu_if_class_loaded:,\whu_if_after_documentclass:}
  \begin{syntax}
    \verb|\whu_if_preamble:TF| \marg{true code} \marg{false code}
  \end{syntax}
测试是否在导言区、正文，或是否已经加载了文档类，或是否在 \tn{documentclass} 后。
\end{function}

\begin{function}[EXP]{\whu_exp_args:Nd,\whu_exp_args:NNd,\whu_exp_args:Nnd,
  \whu_exp_last_unbraced:Nd,\whu_exp_last_unbraced:NNd,\whu_exp_last_unbraced:Nnd}
  \begin{syntax}
    \verb|\whu_exp_args:Nd| \meta{function} \marg{tokens}
    \verb|\whu_exp_args:NNd| \meta{function} \meta{function} \marg{tokens}
    \verb|\whu_exp_args:Nnd| \meta{function} \marg{tokens_1} \marg{tokens_2}
  \end{syntax}
...
\begin{xample}
\ExplSyntaxOn
\whu_exp_args:Nd \tl_to_str:n
  { \prg_replicate:nn { 5 } { \WhuTeX } }
\ExplSyntaxOff
\stopxamplecode
\xampleprint 
\end{xample}
...
\begin{xample}
\ExplSyntaxOn
\whu_exp_last_unbraced:Nd \tl_to_str:n 
...
\begin{function}[EXP]{\whu_serial_exp_args:nnw,\whu_serial_exp_args:eew}
  \begin{syntax}
    \V*|\whu_serial_exp_arg:nnw| \marg{spec_{1m}} \marg{spec_{2}} \meta{tl_{1m}} \marg{tl}
  \end{syntax}
依次执行 
\cs{whu_exp_args:nw} \texttt\{\meta{spec_{1m}}\meta{spec_{2i}}\texttt\} \meta{tl_i} 
...
\whu_serial_exp_arg:nnw { N } { o o f } \tl_to_str:n { \l_tmpa_tl { 3 } }
\ExplSyntaxOff
\stopxamplecode
\xampleprint
\end{xample}
...
\cs{whu_exp_args:nw} \texttt\{\texttt N\meta{spec}\texttt\}。
\end{function}

\begin{function}[EXP]{\whu_use_none_num:nw}
  \begin{syntax}
    \V*|\whu_use_none_num:nw| \marg{num} \meta{tl}
  \end{syntax}
...
\begin{xample}
\ExplSyntaxOn
\whu_exp_args:NNd \tl_set:Nn \l_tmpa_tl 
  { \whu_use_none_num:nw { 5 } 1234567890 }
...
\begin{function}[EXP]{\whu_exp_num:nN,\whu_exp_after_num:nwN}
  \begin{syntax}
    \V*|\whu_exp_num:nN| \marg{num} \meta{token_1}
    \V*|\whu_exp_after_num:nwN| \marg{num} \meta{token_0} \meta{token_1}
  \end{syntax}
...
\begin{function}[EXP]{\whu_tl_use:Nnnn,\whu_tl_use:Nn}
  \begin{syntax}
    \V*|\whu_tl_use:Nnnn| <tl var> <{separator between two}> 
    ~~~~<{separator between more than two}> <{separator between final two}>
    \V*|\whu_tl_use:Nn|   <tl var> <{separator}>
  \end{syntax}
...
\begin{function}[EXP]{\whu_act_case:nnnn,
  ...
  \begin{syntax}
    \V*|\whu_act_case_true:nnnn| \marg{tl} \marg{processor list} \marg{fallback tokens} \marg{act args}
  \end{syntax}
...
\cs{whu_act_case:nnnn} 是 \cs{whu_act_case_true:nnnn} 的另一个名字。
\end{function}
...
\begin{xample}
\ExplSyntaxOn
\exp_args:No \whu_if_lazy_any:nnTF { \token_to_str:N : }
  {
    { \tl_if_eq:nnTF  {:} } % 不满足
    { \str_if_eq:nnTF {:} } % 满足
  }
  ...
  { false }
\ 
\exp_args:No \whu_if_lazy_all:nnTF { \token_to_str:N : }
  {
...
\begin{function}{\whu_map_nest_code:Nnnn,\whu_map_nest_variable:NnnNn}
  \begin{syntax}
    \V*|\whu_map_nest_code:Nnnn|      \meta{map tokens function} \marg{arg} \marg{nest} \marg{code}
    \V*|\whu_map_nest_variable:NnnNn| \meta{map tokens function} \marg{arg} \marg{nest} \meta{variable} \marg{code}
  \end{syntax}
...
\whu_map_nest_code:Nnnn \tl_map_tokens:nn { 01 } { 4 }
...
% \begin{function}{\whu_new_psr:nnn,\whu_set_psr:nnn,\whu_gset_psr:nnn,\whu_use_psr:n}
%   \begin{syntax}
%     \verb|\whu_new_psr:nnn| \marg{处理器} \marg{参数数目} \marg{code}
%     \verb|\whu_use_psr:n|   \marg{处理器}
%   \end{syntax}
% 创建、使用处理器。
...
% \begin{function}{\whu_new_psrrule:nnn,\whu_set_psrrule:nnn,\whu_gset_psrrule:nnn}
%   \begin{syntax}
%     \verb|\whu_new_psrrule:nnn| \marg{处理器} \marg{规则} \marg{code}
%   \end{syntax}
% 创建从属于 \meta{处理器} 的规则 \meta{规则}。该 \meta{规则} 可使用 \meta{处理器}
% 预先定义的参数。
% \end{function}

% \begin{function}{\whu_obey_psrrule:nn,\whu_gbey_psrrule:nn}
%   \begin{syntax}
%     \verb|\whu_obey_psrrule:nn| \marg{处理器} \marg{规则}
%   \end{syntax}
% 对 \meta{处理器} 局部或全局应用 \meta{规则}。注意，全局应用\emph{不是} \verb|gobey|。
% \end{function}

% \begin{function}[pTF]{\whu_psr_if_exist:n,\whu_psrrule_if_exist:nn,\whu_psr_if_compatible:nn}
%   \begin{syntax}
%     \verb|\whu_psr_if_exist:nTF| \marg{处理器} \marg{true code} \marg{false code}
%     \verb|\whu_psrrule_if_exist:nTF| \marg{处理器} \marg{规则} \marg{true code} \marg{false code}
%     \verb|\whu_psr_if_compatible:nnTF| \marg{处理器{}_1} \marg{处理器{}_2} \marg{true code} \marg{false code}
%   \end{syntax}
% 判断处理器、处理器的规则是否存在。或判断两个处理器是否兼容，当前，两个参数数目一致时视为兼容。
% \end{function}

% \begin{function}{\whu_exec_psrrule:nn}
%   \begin{syntax}
%     \verb|\whu_exec_psrrule:nn| \marg{处理器} \marg{规则}
%   \end{syntax}
% 直接执行 \meta{处理器} 的 \meta{规则}。可用于其它规则中，相当于一个宏。其后需有对应数目的参数。
% \end{function}

% \begin{function}[EXP]{\whu_psr_argument_count:n}
%   \begin{syntax}
%     \verb|\whu_psr_argument_count:n| \marg{处理器}
%   \end{syntax}
% 计算处理器可用的参数数目。
% \end{function}

% \begin{function}{
%   \whu_new_psrrule_eq:nnn,
%   \whu_set_psrrule_eq:nnn,
%   \whu_gset_psrrule_eq:nnn
% }
%   \begin{syntax}
%     \verb|\whu_new_psrrule_eq:nnn| \marg{处理器} \marg{规则{}_1} \marg{规则{}_2}
%   \end{syntax}
% 将 \meta{规则{}_1} 设置为与 \meta{规则{}_2} 相等，它们从属于 \marg{处理器}。
% \end{function}

% \begin{function}{
%   \whu_new_psrrule_eq_cs:nnN,
%   \whu_new_psrrule_eq_cs:nnc,
%   \whu_set_psrrule_eq_cs:nnN,
%   \whu_set_psrrule_eq_cs:nnc,
%   \whu_gset_psrrule_eq_cs:nnN,
%   \whu_gset_psrrule_eq_cs:nnc
% }
%   \begin{syntax}
%     \verb|\whu_new_psrrule_eq_cs:nnN| \marg{处理器} \marg{规则} \meta{function}
%   \end{syntax}
...
\section{\whumodule{box}模块}\label{sec:module-box-prog}

\whumodule{box} 模块封装了一些环境或命令，用于在编程时使用。另见 \cref{sec:collectn}。

\subsection{为宽度固定和宽度可变的内容创建超链接}

\whumodule{util} 模块提供了创建超链接的命令。
...
\begin{function}{\whu_ref_label_width:nnnn,\whu_ref_label_varwidth:nnnn,
  ...
  \begin{syntax}
    \V*|\whu_ref_label_width:nnnn| \marg{label} \marg{vpos} \marg{width} \meta{material}
  \end{syntax}
将 \meta{material} 链接到 \meta{label} 或 \meta{target} 的位置。
其宽度（或最大宽度）为 \meta{width}，垂直位置为 \meta{vpos}。

另见 \csref{whu_ref_label_box:nn}、\csref{whu_ref_target_box:nn}。
\end{function}
...
\begin{function}{\whu_set_shbox:Nn,\whu_set_shbox:Nw,\whu_set_shbox_end:}
  \begin{syntax}
    \V*|\whu_set_shbox:Nn| \meta{box} \marg{content}
    \V*|\whu_set_shbox:Nw| \meta{box} \meta{content} \V*|\whu_set_shbox_end:|
  \end{syntax}
...
\begin{function}{\whu_peek_shbox:Nnw}
  \begin{syntax}
    \V*|\whu_peek_shbox:Nnw| \meta{box} \marg{code} \meta{material}
  \end{syntax}
...
\begin{function}{\whu_use_shbox:N}
  \begin{syntax}
    \V*|\whu_use_shbox:N| \meta{box}
  \end{syntax}
类似于 \cs{box_use:N}，使用上述两个命令设置的盒子必须使用这个命令来使用盒子。
\end{function}

注意在使用了 \cs{whu_set_shbox:Nw} 后如果没有使用 \cs{whu_use_shbox:N}，则不能再使用 \cs{whu_set_shbox:Nw}。

使用这个技术使得超链接文本可以包含特殊文本且能够正常断行。

\begin{function}{\whu_ref_label_shbox:nn,\whu_ref_target_shbox:nn}
  \begin{syntax}
    \V*|\whu_ref_label_shbox:nn| \marg{label} \meta{material}
  \end{syntax}
链接到 \meta{label} 或 \meta{target}。\meta{material} 可以包含特殊文本，可以断行，但不允许手动断行。

另见 \cs{whu_ref_label_box:nn}、\cs{whu_ref_target_box:nn}。
\end{function}



\section{\whumodule{struct}模块}\label{sec:struct-programming}

以下简单描述 \whumodule{struct} 模块中目录的数据结构。
...
绝大多数情况下，无需手动设置它，\whumodule{struct} 模块会自动设置它们。
...
{\centering \fbox{\parbox{.93\textwidth}{\cs{whu@type@contentsline}
  \marg{type}\marg{cbl count}\marg{info}\marg{level}\hfil\break
  \null\quad\marg{cbl list entry}\marg{thepage}\marg{anchor}\cs{whu@type@contentsline@}}}\par}

\begin{itemize}[nosep]
  \item \cs{whu@type@contentsline}，\cs{whu@type@contentsline@}，这两个宏标记条目的边界；
...
cbl 列表除了 \cs{whu@type@contentsline}、\cs{whu@type@contentsline@}、
\meta{cbl count} 改为 \cs{whu@cbl@contentsline}、\cs{whu@cbl@contentsline@}、
...
\begin{function}{\whu_contents_get:nN,\whu_contents_type_get:nnN}
  \begin{syntax}
    \V*|\whu_contents_get:nN| \marg{cbl count} \meta{tl}
    \V*|\whu_contents_type_get:nnN| \marg{type} \marg{type count} \meta{tl}
  \end{syntax}
...
\begin{function}{\whu_get_heading_level:nnN}
  \begin{syntax}
    \V*|\whu_get_heading_level:nnN| \marg{type} \marg{level name} \meta{tl}
  \end{syntax}
...
\WhuTeX 的目录机制适配了许多宏包，诸如 \pkg{algorithm2e}、\pkg{chemmacros}、
...
\pkg{etoc} 宏包提供了 \tn{etocsetstyle} 命令来设置目录，\WhuTeX 提供了类似的命令。

\WhuTeX 提供了 \csref{tocsetstyle} 和 \csref{specifiedlot} 来分别设置和输出目录，
...
下面这个例子展示了本文目录在 \WhuTeX 目录机制下的输出顺序。
\begin{xample}
...
在 \WhuTeX 中，如果要输出某类目录，则必须先通过 \csref{addcombinedlisttype} 注册它，
...
由于 \WhuTeX 把所有类型的目录都写入到同一个文件中，因此，\veta{type} 
用于唯一区分不同的目录类型。

同一个目录中，可以有不同的层级，同一层级也可以有所区分。

在 \WhuTeX 中，用于区分不同层级的
...
但在 \WhuTeX 宏集中，每个 \veta{level name} 只需在其所属的那个目录类型中唯一，
...
尽管看起来很复杂，但实际上以上这些工作在绝大多数情况下 \WhuTeX 都会自动完成它们，
无需手动设置它。


\chapter{库的文档接口}

\section{\whulibrary{pgf}库}

\whulibrary{pgf} 库使用 \pkg{pgf} 宏集的功能，定义了一些命令。

\whulibrary{pgf} 库\emph{不会}自动加载 \pkg{pgf} 和 \pkg{tikz} 宏包，需要用户自行加载。
...
\section{\whulibrary{tcb}库}

\whulibrary{tcb} 库使用 \pkg{tcolorbox} 宏包的功能，定制了一些其它功能。用户
...
\section{\whulibrary{doc}库}

\whulibrary{doc} 库用于支持排版说明文档。本库移植修改自 \cls{l3doc} 和 \cls{ctxdoc}。
...
\cs{whu@doc@cs@format} 等命令则是修改显示的格式，最多可带有一个参数。
这些命令中可以修改链接显示的颜色和字体。
\end{function}
...
\cs{whu@doc@meta@format} 等命令用于修改显示的格式。
...
\begin{syntax}
  ...
  rEXP &&
\end{syntax}
\opt{EXP} 将函数标记为\emph{完全可展的}（fully expandable functions），可同时用作 \verb|x|、\verb|e|、\verb|f| 类型的参数。如 \tn{string}、\cs{cs_to_str:N}。使用 \UseName{\detokenize{__whu_doc_typeset_exp:}} 标记。

\opt{rEXP} 将函数标记为\emph{受限可展的}（restricted expandable functions），
这些函数是完全可展的，但不能在 \verb|f| 类型的参数中完全展开（cannot be fully expanded）。
如 \cs{seq_map_function:NN}。使用 \UseName{\detokenize{__whu_doc_typeset_rexp:}} 标记。
\end{keyval}
...
\begin{function}{\csref,\csreflist,\envref,\envreflist,\keyref,\keyreflist,
  ...
  \end{syntax}
引用命令，环境或键。对于列表的引用，可以通过 \cs{whu@doc@refrange} 修改分隔字符。

\cs{whu@doc@csref@format} 等三个命令则是修改它们显示的格式，最多可带有一个参数。

\meta{type} 为 \keyref[doc/cmd]{type} 设置的值。
\end{function}

\begin{function}{\whu@doc@ttfont,\whu@doc@itfont}
这两个命令分别用于设置 \whulibrary{doc} 库中使用的等宽字体和斜体。
\end{function}
...
\section{\whulibrary{bnf}库}

\whumodule{bnf} 库用于排版基于 Backus-Naur Form（BNF 范式）的文法。
...
\begin{keyval}[path=texbnf]{Nleft,Nright,Tleft,Tright,N,T,O,I}
  \begin{syntax}
    ...
    O = \marg{code} & \V{\whu@mathrule}, \UseName{whu@mathrule}
  \end{syntax}
...
\section{\whulibrary{ref}库}\label{sec:lib-ref}

\whumodule{ref} 库提供交叉引用的一些额外功能。
...
另见 \csref{whu_ref_label_box:nn}、\csref{whu_ref_target_box:nn} 和 
\csref{whu_ref_label_shbox:nn}、\csref{whu_ref_target_shbox:nn}。
\end{function}
...
虽然这两个命令不能配置 \meta{material}，但 \whulibrary{box} 库提供了 \csref{fparbox} 和 \csref{fvarbox} 命令可以设置盒子外框，见\cref{sec:fparbox-fvarbox}及\cref{eg:hyperref-fparbox}。


\section{\whulibrary{box}库}\label{sec:lib-box}

\whumodule{box} 库提供额外的一些盒子。
...
\begin{itemize}[midpenalty=-1000,
  ...
  \item 更好的文档；
  \item 实现 \whumodule{pgf} 库；
  \item 适配 \pkg{tcolorbox}；
  \item 提供 \pkg{cleveref} 宏包的接口；
  \item \cls{whuclass} 文档类；
  \item 更好的竖排文档支持；
  \item \whumodule{struct} 模块支持 \pkg{titlesec} 的所有标题形状；
  \item 更多丰富的标题设置；
  \item \whumodule{struct} 模块支持 \hologo{KOMAScript} 的目录设置方式；
  \item 优化现有的局部目录实现方式；
  \item \whumodule{struct} 模块简单支持 \pkg{titletoc} 的目录设置方式；
  \item 适配 latex-lab；
  \item 文档部件（frontmatter，mainmatter，appendix 等）；
  \item 注记支持（脚注、边注、尾注等）；
  \item \whulibrary{colorful} 库，彩色；
  \item \whulibrary{textdeco} 库，文字装饰；
  \item 完善 \whulibrary{doc} 库；
  \item 子文档；
  \item 更加适配参考文献；
  \item 更加适配术语表；
  \item 实现 \whulibrary{hooks} 库；
  \item 实现 \whulibrary{external} 库；
  \item tagged pdf；

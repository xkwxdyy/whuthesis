...
\WHUDependency{package={xcolor,etoolbox,underscore,
  ...
  module={index.cus,box.cus},library={logo.cus}}
\WHUProvideExplLibrary{doc.cus}{\whu@d@te}{\whu@versi@n}{documentation}

\keys_define:nn { whu/doc-options }
  {
    no-index-file .bool_set_inverse:N = \l__whu_doc_index_file_bool,
    no-index-file .initial:n = false,
  }

\ProcessKeyOptions[whu/doc-options]
...
\bool_if:NT \l__whu_doc_index_file_bool
  {
...
\whusetstyle[frame]{doc~color~frame}{
  frame={\fboxsep\whuframesep \fboxrule\whuframerule 
    \fcolorbox{whu/color/doc~frame}{whu/color/doc~fill}}
}
\whusetstyle[frame]{doc~function~style}{outer-sep=0pt, doc~color~frame,
  doc~frame~color=blue, doc~fill~color=blue!20, sep=1ex}
\whusetstyle[frame]{doc~syntax~style}{outer-sep=0pt, doc~color~frame,
  doc~frame~color=red!20, doc~fill~color=red!20, sep=.5ex}
\whusetstyle[frame]{doc~description~style}{outer-sep=0pt, doc~color~frame,
...
\bool_set_eq:NN \l__whu_doc_allow_indexing_bool \l__whu_doc_index_file_bool
...
\cs_generate_variant:Nn \__whu_doc_get_hyper_target:nN { o , e }
...
\bool_if:NF \l__whu_doc_index_file_bool
  { \cs_set_protected:Npn \__whu_doc_index_page_hc:nn #1#2 { } }
\group_begin:
...
\cs_new_protected:Npn \__whu_doc_hdindex:nn #1#2
  {
    ...
    \tl_replace_all:Nen \l__whu_doc_tmpa_tl
...
\bool_if:NF \l__whu_doc_index_file_bool
  { \cs_set_protected:Npn \__whu_doc_ltx_changes:nnn #1#2#3 { \group_end: \@esphack } }
...
\cs_new_protected:Npn \__whu_doc_names_get_seq:nN #1#2
  {
    ...
    \bool_if:NTF \l__whu_doc_names_verb_bool
      ...
      {
        \tl_remove_all:Ne \l__whu_doc_tmpa_tl
          { \iow_char:N \^^M \c_percent_str }
        \tl_remove_all:Ne \l__whu_doc_tmpa_tl
          { \token_to_str:N \obeyedline \c_space_tl \c_percent_str }
        \tl_remove_all:Ne \l__whu_doc_tmpa_tl
          { \token_to_str:N \obeyedline \c_space_tl }
        \tl_remove_all:Ne \l__whu_doc_tmpa_tl { \tl_to_str:n { ^ ^ A } }
        \tl_remove_all:Ne \l__whu_doc_tmpa_tl { \iow_char:N \^^I }
        \tl_remove_all:Ne \l__whu_doc_tmpa_tl { \iow_char:N \^^M }
...
\cs_new_protected:Npn \__whu_doc_names_typeset_auxii:n #1
  {
    \bool_if:NT \l__whu_doc_macro_pTF_bool
      {
        \__whu_doc_names_typeset_block:eN
...
\cs_generate_variant:Nn \__whu_doc_names_typeset_auxii:n { e }
...
\cs_generate_variant:Nn \__whu_doc_names_typeset_block:nN { e }
...
\cs_new_protected:Npn \__whu_doc_function_descr_start:w
  {
    \vcoffin_set:Nnw \l__whu_doc_descr_coffin { \textwidth }
      \whu_box_start:nn { width=\textwidth, doc~description~style }
        ...
        \tl_if_empty:NF \l__whu_doc_date_added_tl
          {
            \legacy_if:nTF { @twoside } 
              ...
              {
                \tl_put_right:Nn \l__whu_doc_tmpa_tl
                  {
                    \multicolumn { 2 } { @{} l @{} } 
